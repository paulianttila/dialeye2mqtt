...
class MyApp:
    ...
    def update(self):
        ...
        if retval == 0:
            ...
            self.logger.debug('m3=%d, litre=%.2f, already_increased=%r, previous_value=%.5f m3, current_value=%.5f m3, consumption=%.2f l', 
                self.m3, litre, self.already_increased, self.previous_value, current_value, consumption)

            if consumption >= 0:
                instant_consumption_l_per_min = self.calc_instant_consumtion(consumption)
                self.logger.info('Current value = %.5f m3, consumption = %.2f l/min (%.2f l)', current_value, instant_consumption_l_per_min, consumption)
                self.publish_values(current_value, instant_consumption_l_per_min)
            else:
                self.logger.error('Consuption %.2f is less than 0, ignore update (current_value=%.5f m3, previous_value=%.5f m3)',
                    consumption, current_value, self.previous_value)
                self.publish_zero_consumption()

            self.write_data_file(self.config['DATA_FILE'], "%d;%r;%f" % (self.m3, self.already_increased, current_value))
    ...
    def get_dialeye_value(self):
        ...
        retval, result = self.execute_command(
            [
                self.config['DIALEYE_PYTHON'], self.config['DIALEYE'],
                '-f', self.config['CONF_FILE'],
                '-s',
                '-u',
                'meter', self.config['IMAGE_URL']
            ],
            timeout=self.config['TIMEOUT']
            )
        end = time.time()
        self.logger.debug('DialEye result (retval=%d, time=%f): %s', retval, (end - start), result)
        litre = None
        if retval == 0:
    ...
    def calc_instant_consumtion(self, consumption):
        ...
        if consumption > 0 and self.previous_time > 0:
            instant_consumption_l_per_min = consumption / (now - self.previous_time) * 60
    ...
    def init_values(self, filename):
        ...
        self.logger.info('Initial values: m3=%d, already_increased=%r, previous_value=%f',
            self.m3, self.already_increased, self.previous_value)
